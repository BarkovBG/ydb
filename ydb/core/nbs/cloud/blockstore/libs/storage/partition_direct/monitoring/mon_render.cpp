...
void RenderHeader(IOutputStream& str, const THeaderInfo& h)
{
    HTML (str) {
        TAG (TH3) {
            str << "partition_direct tablet " << h.TabletId;
        }
            ...
            TABLEBODY () {
                if (d.Runtime) {
                    ...
                    TABLER () {
                        ...
                        TABLED () {
                            str << OptLsn(r.GlobalSafeBarrier);
                        }
                    }
                } else {
                    TABLER () {
    ...
    HTML (str) {
        ...
        TABLE_CLASS ("table table-condensed") {
            TABLEBODY () {
                ...
                TABLER () {
                    TABLED () {
                        str << "DirectBlockGroupsConnections";
                    }
                    TABLED () {
                        str << (db.HasConnections ? "" : "(none)");
                        str << "<pre>" << HtmlEscape(db.ConnectionsText)
                            << "</pre>";
                    }
                }
            }
            ...
            TABLEBODY () {
                for (const auto& row: db.VChunkConfigs) {
                    TABLER () {
                        ...
                        TABLED () {
                            str << "<pre>" << HtmlEscape(row.Summary)
                                << "</pre>";
                        }
                    }
                }
                ...
                TABLEBODY () {
                    for (const auto& c: dbg.Connections) {
                        TABLER () {
                            ...
                            TABLED () {
                                str << HtmlEscape(c.DDiskId);
                            }
                            TABLED () {
                                str << HtmlEscape(c.PBufferId);
                            }
                            TABLED () {
                                str << HtmlEscape(c.DDiskSession);
                            }
                            TABLED () {
                                str << (c.PBufferConnected ? "yes" : "no");
                            }
                        }
                    }
                }
                ...
                TABLEBODY () {
                    for (const auto& h: dbg.Hosts) {
                        TABLER () {
                            ...
                            TABLED () {
                                str << HostStateName(h.State);
                            }
                            TABLED () {
                                str << HealthName(h.Health);
                            }
                            TABLED () {
                                str << h.PBufferUsedSize;
                            }
                            TABLED () {
                                str << h.Errors.ErrorCount;
                            }
                            for (size_t op = 0; op < OperationCount; ++op) {
                                TABLED () {
                                    str << h.InflightByOp[op];
                                }
                            }
                        }
                    }
    ...
    HTML (str) {
        ...
        for (const auto& dbg: r.Dbgs) {
            ...
            for (const auto& vc: dbg.VChunks) {
                ...
                TAG (TH4) {
                    str << "DBG #" << dbg.Index << " vchunk #" << vc.Index;
                }
                TABLE_CLASS ("table table-condensed") {
                    TABLEBODY () {
                        TABLER () {
                        ...
                        for (size_t host = 0; host < vc.HostCount; ++host) {
                            TABLER () {
                                ...
                                TABLED () {
                                    str
                                        << (host < vc.Roles.size()
                                                ? vc.Roles[host].PBufferRole
                                                : TString("-"));
                                }
                                TABLED () {
                                    str
                                        << (host < vc.Roles.size()
                                                ? vc.Roles[host].DDiskRole
                                                : TString("-"));
                                }
                                TABLED () {
                                    str
                                        << (host < vc.Roles.size() &&
                                                    vc.Roles[host].Enabled
                                                ? "yes"
                                                : "no");
                                }
                                TABLED () {
                                    str
                                        << (host < vc.Roles.size()
                                                ? OptLsn(
                                                      vc.Roles[host].Watermark)
                                                : TString("-"));
                                }
                                TABLED () {
                                    str
                                        << (host < vc.DDiskStates.size()
                                                ? vc.DDiskStates[host].State
                                                : TString("-"));
                                }
                                TABLED () {
                                    str
                                ...
                                TABLED () {
                                    str
                                        << (host < vc.PBuffers.size()
                                                ? ToString(vc.PBuffers[host]
                                                               .CurrentRecords)
                                                : TString("-"));
                                }
                                TABLED () {
